...
function bigProfileCardHTML(p) {
  ...
  const photoHTML = p.photo
    ...
    : `<span>${p.emoji}</span>`;
  return `
    <div class="profile-big-card" data-id="${p.id}" onclick="navigate('profile-detail',{profile:this.closest('[data-id]').dataset.id})">
...
function renderProfileDetail(id) {
  ...
  document.getElementById('pd-neighbours').innerHTML = nb.map(n => `
    <div style="display:grid;grid-template-columns:30px 28px 1fr auto;gap:6px;align-items:center;padding:6px 0;border-bottom:1px solid var(--border);cursor:pointer" data-id="${n.id}" onclick="navigate('profile-detail',{profile:this.dataset.id})">
...
function renderRatingTable(tab) {
  ...
  document.getElementById('ratingTableBody').innerHTML = list.map((p, i) => `
    <tr data-id="${p.id}" onclick="navigate('profile-detail',{profile:this.dataset.id})" style="cursor:pointer">
